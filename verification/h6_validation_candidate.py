...
class H6ValidationPerturbationArtifactReference:
    """Exact identity of one immutable full perturbation candidate."""
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.vocabulary_id != _VOCABULARY_ID
            or self.vocabulary_size != _VOCABULARY_SIZE
            or type(self.vocabulary_size) is not int
            or self.tokenizer_spec_sha256 != _TOKENIZER_SPEC_SHA256
            or self.vocabulary_sha256 != _VOCABULARY_SHA256
            or self.perturbation_schema_version != _PERTURBATION_SCHEMA
            or self.generator_version != _PERTURBATION_SCHEMA
            or self.seed != _SEED
            or type(self.seed) is not int
            or self.full_count != _FULL_COUNT
            or type(self.full_count) is not int
            or self.materialized_count != _FULL_COUNT
        ...
        for path, value in self.payload_sha256s:
            if type(path) is not str:
                raise H6ValidationCandidateError(
                    "artifact reference payload path is invalid"
                )
            _require_sha256(value, f"payload SHA-256 for {path}")
        candidate = _ValidatedCandidate(
            canonical_bytes=b"",
            raw_sha256=self.perturbation_raw_sha256,
            inner_manifest_sha256=(
                self.perturbation_inner_manifest_sha256
            ),
            schema_version=self.perturbation_schema_version,
        ...
        fixture = SimpleNamespace(
            reference_sha256=self.validation_fixture_reference_sha256,
            binary_directory_manifest_sha256=(
        ...
        expected = _artifact_reference_hash(
            source=source,
        ...
        if self.reference_sha256 != expected:
            raise H6ValidationCandidateError(
                "artifact reference SHA-256 is stale"
            )
