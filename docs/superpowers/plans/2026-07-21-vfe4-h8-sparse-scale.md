...
- The parent decodes an accepted production/profiler v2 PASS body into a separate private typed `H8DecodedPassEvidence` attached to its parent attempt. That private record retains sample-noise identity, exact problem evidence, diagnostics, the complete allocation mapping, and exact request/envelope/result identities. It supplies the lossless factor/allocation runtime views; it does not expand public `H8ChildResult` or the published `production_runs`/`profiler_runs` schemas.
...
- `validation/h8.json` is the normative in-artifact structured result under `schema_version="h8-sparse-scale-v4"`. V4 supersedes the underspecified staged v3 contract before any H8 scientific execution. It binds exact source revision and dirty-content digest, canonical H8 config hash, prerequisite artifact/hash set, manuscript/preregistration hashes, parent-owned child attempts, child protocol and environment, raw run/correctness/allocation records, statuses/obligations, and explicit H8 nonclaims. It contains no hash of its own enclosing manifest. Only after atomic publication, write `.verification/h8-current-candidate-<FULL_HEAD>-result.json` with the artifact path plus candidate, manifest, config, validation, JUnit, reference-registry, and predecessor hashes; the H8 ledger must re-read and independently revalidate this external pointer.
...
- [ ] **Step 7: Run focused GREEN child tests.**
  ...
  Expected: PASS. No production-size 30-attempt runtime execution yet.
...
manifest; Task 7B added the injected issued-prefix runner. No H8 scientific
workload has run. Remaining work must execute strictly in this order:
...
- [ ] **Step 5: Build, independently compare, and freeze `validation/h8.json`.** The H8 gate owns runtime-section construction from the exact typed child/problem evidence; no caller-supplied or preassembled runtime mapping can authorize status. Independently reconstruct and compare every field, run key, hash, schema literal, inventory order, resource sentinel, and cross-binding before publication. Set `schema_version="h8-sparse-scale-v4"` and `gate="H8"`. Reject unknown/missing keys at every level. The exact top-level key order is `schema_version`, `gate`, `status`, `obligations`, `bounded_claim`, `nonclaims`, `revision`, `config`, `prerequisites`, `interpretation`, `protocol`, `environment`, `problems`, `storage`, `factor`, `correctness`, `allocation`, `controls`, `child_attempts`, `production_runs`, `profiler_runs`, `budgets`, `invariants`, and `artifacts`. Exact nested key sets are:
  ...
  - `child_attempts` is the parent-owned frozen-order prefix, up to the exact 30-record inventory: 15 production requests in `(seed,repetition)` seed-major order, three profiler requests in seed-major order, then 12 negative-control requests in the frozen control order with `seed=20260721` and `repetition=null`. A witnessed FAIL may close the array early; PASS requires all 30. Each attempt is exactly `{request,status,reasons,result_kind,result_identity,nonpass_envelope,timed_out,exit_code,parent_elapsed_ns,request_sha256,identities_sha256,stdout_sha256,stderr_sha256,operation_reachability,residuals,resource_decisions}`. `result_kind` is `child`, `control`, or null; the corresponding optional identity is exactly `{mode,seed,repetition,input_sha256}`, `{control_id,event_sha256}`, or null. A parseable envelope from a timed-out launch, a parseable child-authored FAIL/INCONCLUSIVE envelope, or a parseable envelope whose identity is rejected, is retained immutably in `nonpass_envelope`; malformed or unavailable stdout uses null and remains bound by `stdout_sha256`. `request_sha256` binds the canonical stdin request, `identities_sha256` binds the exact canonical `VFE4_H8_CHILD_IDENTITIES_JSON` bytes, and the stdout/stderr hashes bind the raw captured streams even when parsing fails. Reachability, residual, and resource-decision maps are optional trusted raw evidence and use null when unavailable, inapplicable, or identity-rejected; missing required evidence contributes the appropriate attempt/gate reason rather than a fabricated map;
  - `production_runs` is the exact 15-record seed-major decoded `H8ChildResult` array and `profiler_runs` the exact three-record seed-major decoded `H8ChildResult` array; these remain the unchanged authoritative ordering and published typed-result inventories. The exact v2 raw body is decoded separately into private typed `H8DecodedPassEvidence` attached to the result-bearing attempt; its sample noise, problem evidence, diagnostics, complete allocation mapping, and exact identities are not duplicated into public `H8ChildResult`. The factor/allocation `runs` arrays are lossless evidence views derived from that private evidence and cross-bound to the unchanged public arrays and their attempts; they do not replace them. `controls` remains the exact 12-record decoded `H8ControlResult` array under the same v2 envelope without fabricated production `problem_evidence`. Every public decoded record equals the public typed result retained by its result-bearing attempt. Its nested `resources` object remains the child's exact public output, including `resources.parent_elapsed_ns=0`; actual parent spawn-through-parse timing remains only on the attempt;
