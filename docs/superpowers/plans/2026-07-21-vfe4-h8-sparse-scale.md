...
- The next parent-orchestrator slice creates and tracks `verification/fixtures/h8_exact_test_nodes_v1.txt`. Every H8 RED/GREEN or milestone pytest instruction below consumes only the exact `::` node IDs assigned to that step in this manifest; no instruction authorizes a whole test file or unfiltered full suite. Until the manifest exists and is tracked, those test steps are blocked. This documentation amendment does not create or execute it.
...
- [ ] **Step 2: Write RED parent-orchestration and attempt-inventory tests.** Create and track `verification/fixtures/h8_exact_test_nodes_v1.txt` as part of this next parent-orchestrator slice; it contains the ordered exact pytest node IDs owned by the slice and the later milestone, rejects duplicate or whole-file entries, and is hashed before either test phase. This documentation amendment deliberately does not create it. Traverse seed order `20260721..23`, repetitions `0..4`; create a fresh process for every production run; run one separate profiler child per seed; then run each negative control in isolation in the frozen order with seed `20260721`. The parent preallocates the exact 30-request sequence and emits one `H8ChildAttemptRecord` for every launch actually issued, including timeout, abnormal exit, malformed stdout, and missing typed result. A witnessed FAIL may stop later launches and leave the exact issued prefix; PASS requires all 30. Enforce parent timeout `60.0`; reject duplicate/reordered attempts, a non-prefix inventory, warm reuse, a result from another request/config/protocol hash, any decoded result not cross-bound to its result-bearing attempt, or aggregation that discards any `PREEXISTING`/`CREATE`/`INCREMENT_VERSION`/`DESTROY` source row, TensorKey+version/dtype/join witness, preexisting baseline field, or raw endpoint.

- [ ] **Step 3: Implement scientific gate preflight and parent child orchestration.** The scientific H8 path parses all immutable problem/reference inputs, runs the literal 12-cell grid through production plus the two bounded verification-only dense references, proves `vfe4/**` cannot import either reference, verifies control detectors, and only then starts the frozen child-attempt sequence. Parent constructs and hashes exact hardware/affinity/thread/BLAS identities before child import; every child sets/verifies both PyTorch thread counts and returns matching hashes. Capture `perf_counter_ns` from spawn through parse into `attempt.parent_elapsed_ns`, including failed parse and timeout paths. Preserve the child-authored `H8ResourceRecord` byte-for-byte, including its protocol sentinel `resources.parent_elapsed_ns=0`; never replace that field with parent timing. `verification/h8_preflight.py` remains the separate standard-library-only, metadata-only, zero-compute advisor: it launches no correctness cell, child, profiler, control, or test and only reports whether this orchestrator is present. Remove `h8_parent_orchestrator_not_implemented` only in this slice after the complete parent path and independent runtime revalidation exist. Never retry a scientific failure; any retry would change the frozen 30-request protocol and therefore yields `INCONCLUSIVE` pending a new preregistration.
...
- [ ] **Step 5: Freeze and test `validation/h8.json`.** Set `schema_version="h8-sparse-scale-v3"` and `gate="H8"`. Reject unknown/missing keys at every level. The exact top-level key order is `schema_version`, `gate`, `status`, `obligations`, `bounded_claim`, `nonclaims`, `revision`, `config`, `prerequisites`, `interpretation`, `protocol`, `environment`, `problems`, `storage`, `factor`, `correctness`, `allocation`, `controls`, `child_attempts`, `production_runs`, `profiler_runs`, `budgets`, `invariants`, and `artifacts`. Exact nested key sets are:
...
- [ ] **Step 9: Run the tracked exact-node GREEN selection.**

  Run only the exact Task 7 node IDs listed in the tracked
  `verification/fixtures/h8_exact_test_nodes_v1.txt`. Every selected entry must
  contain a pytest `::` node delimiter; do not select an entire test file or an
  unfiltered suite.
