...
- The canonical parent/child protocol digest preimage freezes `factor_schema="h8-block-tridiagonal-cholesky-v1"`, `selected_inverse_schema="h8-block-takahashi-selected-inverse-v1"`, `condition_estimator_schema="HagerHigham1NormEstimate-v1"`, `allocation_schema="h8-allocation-observability-v1"`, `profiler_raw_event_schema="h8-torch-profiler-raw-event-v1"`, and `child_schema="h8-child-v2"`. It also includes the complete required-operation and ordered negative-control inventories and every frozen numerical and boundary constant. Parent and child must recompute the same digest from the complete preimage; an omitted or substituted literal, inventory, or constant is `INCONCLUSIVE`.
- Bump `H8ValidationConfig` from legacy `h8-validation-config-v1` to `schema_version="h8-validation-config-v2"`. V2 contains the six frozen fields `factor_schema="h8-block-tridiagonal-cholesky-v1"`, `selected_inverse_schema="h8-block-takahashi-selected-inverse-v1"`, `condition_estimator_schema="HagerHigham1NormEstimate-v1"`, `allocation_schema="h8-allocation-observability-v1"`, `profiler_raw_event_schema="h8-torch-profiler-raw-event-v1"`, and `child_schema="h8-child-v2"`. All six enter canonical resolved-config JSON, so the canonical H8 config SHA changes from the v1 configuration identity accordingly.
...
- `validation/h8.json` is the normative in-artifact structured result under `schema_version="h8-sparse-scale-v4"`. V4 supersedes the underspecified staged v3 contract before any v4 scientific or milestone execution may be accepted as promotion evidence. It binds exact source revision and dirty-content digest, canonical H8 config hash, prerequisite artifact/hash set, manuscript/preregistration hashes, parent-owned child attempts, child protocol and environment, raw run/correctness/allocation records, statuses/obligations, and explicit H8 nonclaims. It contains no hash of its own enclosing manifest. Only after atomic publication, write `.verification/h8-current-candidate-<FULL_HEAD>-result.json` with the artifact path plus candidate, manifest, config, validation, JUnit, reference-registry, and predecessor hashes; the H8 ledger must re-read and independently revalidate this external pointer.
...
- [ ] **Step 7: Run focused GREEN child tests.**
  ...
  milestone evidence; no v4 scientific/milestone execution from it may be
...
manifest; Task 7B added the injected issued-prefix runner. No v4
...
- [ ] **Step 9: Run one focused tracked exact-node JUnit selection.**
  ...
  First add every new v4-contract test as an exact node ID in the tracked
