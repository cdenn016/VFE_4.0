...
- [ ] **Step 3: Implement lossless child evidence and exact problem endpoints.** Extend the typed child result so every production and profiler result retains complete fill, workspace, `SparseConditionDiagnostics`, counters, reconstruction invariants, complete validated child allocation mapping, and exact child-authored `H8ResourceRecord`. Never reduce allocation to `H8AllocationRecord`, select the first run, retain only a maximum, or rewrite `resources.parent_elapsed_ns=0`. Persist profiler keys as `{tensor_id,storage_ptr,allocation_id,device}`. Freeze the `h8-generative-evidence-v1`, `h8-recognition-evidence-v1`, `h8-local-spd-diagnostics-v1`, `h8-transition-norms-v1`, and observation hash preimages exactly as specified in v4. Capture transition norms from the already required contraction calculation without a second per-child SVD pass. Reconstruct each seed's problem record only after byte-identical agreement across all five production repetitions and that seed's profiler result; disagreement is `INCONCLUSIVE`.
...
- [ ] **Step 5: Build, independently compare, and freeze `validation/h8.json`.** The H8 gate owns runtime-section construction from the exact typed child/problem evidence; no caller-supplied or preassembled runtime mapping can authorize status. Independently reconstruct and compare every field, run key, hash, schema literal, inventory order, resource sentinel, and cross-binding before publication. Set `schema_version="h8-sparse-scale-v4"` and `gate="H8"`. Reject unknown/missing keys at every level. The exact top-level key order is `schema_version`, `gate`, `status`, `obligations`, `bounded_claim`, `nonclaims`, `revision`, `config`, `prerequisites`, `interpretation`, `protocol`, `environment`, `problems`, `storage`, `factor`, `correctness`, `allocation`, `controls`, `child_attempts`, `production_runs`, `profiler_runs`, `budgets`, `invariants`, and `artifacts`. Exact nested key sets are:
  ...
  - `production_runs` is the exact 15-record seed-major decoded `H8ChildResult` array and `profiler_runs` the exact three-record seed-major decoded `H8ChildResult` array; these remain the authoritative ordering and published typed-result inventories. The factor/allocation `runs` arrays are lossless evidence views cross-bound to those arrays and their result-bearing attempts and do not replace them. `controls` remains the exact 12-record decoded `H8ControlResult` array. Every decoded record equals the typed result retained by its result-bearing attempt. Each production/profiler child record uses the `H8ChildResult` field set plus `{parent_elapsed_ns,child_elapsed_ns,exit_code,stdout_sha256,stderr_sha256,operation_reachability,residuals,resource_decisions}`. Its nested `resources` object remains the child's exact output, including `resources.parent_elapsed_ns=0`; the actual spawn-through-parse value is copied only from the parent attempt into the child record's top-level `parent_elapsed_ns`;
