...
## Task 11 prerequisite-registry amendment (2026-07-28)

H8 now authorizes only `h8-current-candidate-refs-v5` with an exact paired
direct-A0 Prefix certificate and Prediction-v3 predecessor. Registry v1, v2,
v3, and v4 remain byte-readable for
historical diagnosis and always carry an unmet prerequisite obligation. The
v5 adapter uses native authenticated readers for the six-file Prefix authority,
config/matching/readiness/plan authority bundle, validation bundle and
checkpoint selection, reservation and FINALIZED journal aliases, exact
4,104-row raw inventory, metrics, result, pointer, and ledger.

This is a prerequisite-identity amendment outside the frozen scientific H8
section. It leaves `H8ValidationConfig`, the frozen scientific configuration
hash, numerical limits, operational interpretation, and status rules
unchanged. Every active type, implementation, and execution step below uses
v5; v1--v4 appear only as explicitly nonauthorizing parser history.
...
**Tech Stack:** Python 3.10+, PyTorch, NumPy, standard-library `subprocess`/`ctypes`/`resource`, pytest with JUnit XML, existing atomic JSON artifacts, installed evidence-gated verification ledger.
...
- Execute H8 only after compatible current H1, H2, H3, H4, H5, H6-Prefix, executable H6-Prediction v3, and H7 evidence is PASS. The exact verifier prefix is `("H1","H2","H3","H4","H5","H6-Prefix","H7","H8")`; H6-Prediction remains a separate v4 reference rather than a member of that verifier tuple. H7 transitively binds H1--H5 and the independently produced H6-Prefix sibling reference; H8 separately validates H7 and the exact Prediction-v3 native authority/transaction/closure chain. H6-Prefix has no H1--H5/H4 predecessor. Prediction readiness binds H1/H2/H3/H5 and never H4 timing. Missing, stale, differently configured, differently sourced, hash-incompatible, or historical Prediction-v1/v2 evidence makes H8 `INCONCLUSIVE` before the child suite starts.
- Because H8 changes H7-owned config/runner/provenance dependencies, the final H8 candidate reproduces the complete final-H7 current-candidate lifecycle, in this exact order after one JUnit XML and with no tracked edit: H1--H5 artifact/ledger; active H1-prefix-prior artifact/ledger; independently projected H6-Prefix artifact/paired bounded and direct-A0 certificates/ledger with `predecessor_refs={}`; H7 predecessor registry; H7 artifact; external H7 result pointer; H7 ledger; H8-v5 predecessor registry; H8 artifact; external H8 result pointer; H8 ledger. Prediction-v3 is not rerun merely for H8, but it is eligible only through `H8H6PredictionV3Reference`: native authorities/readiness/plan/matching and validation/checkpoint reopen, authenticated reservation-derived output paths, exact raw/metrics/result/pointer identities, the independently pinned validator digest, live artifact revision, and the specific hash-bearing closure claim. Prediction-v1/v2 records and the five-file registry-v4 Prefix record are historical parse inputs and are never execution inputs.
...
- The canonical parent/child protocol digest domain is `vfe4.h8.parent-child-protocol.v3`; its preimage freezes `factor_schema="h8-block-tridiagonal-cholesky-v1"`, `selected_inverse_schema="h8-block-takahashi-selected-inverse-v1"`, `condition_estimator_schema="HagerHigham1NormEstimate-v1"`, `allocation_schema="h8-allocation-observability-v1"`, `profiler_raw_event_schema="h8-torch-profiler-raw-event-v1"`, and the unchanged child envelope `child_schema="h8-child-v2"`. It also includes the four installed-runtime profiler identity fields, the complete required-operation and ordered negative-control inventories, and every frozen numerical and boundary constant. Parent and child must recompute the same digest from the complete preimage; an omitted or substituted literal, inventory, or constant is `INCONCLUSIVE`.
- `h8-validation-config-v2` and the 2.9.1/v4 contract are superseded pre-outcome history. The active `H8ValidationConfig` is `schema_version="h8-validation-config-v3"`, binding the six frozen schema fields plus exact `torch.__version__="2.10.0.dev20251210+cu128"`, `_memory_profiler.py` SHA-256 `22de3b0790907b90053af829ebf1bff0b6add2745ac0381ec7de78812edacb47`, `profiler.py` SHA-256 `543430b2e9b24df777f86415865fee250b35e3444a80920bcca0e8889b917956`, and descriptor SHA-256 `2ee166166bab997499cc66da85146a031f458fbe0190a75b1a1a3ddea80efc38`. All enter canonical resolved-config JSON and cross-bind to v3 wire authority.

All earlier Task 1--6 2.9.1/v4 implementation instructions retained below are
superseded pre-outcome history only; this v5/config-v3/protocol-v3 amendment
is the active contract.
- Every production/profiler `h8-child-v2` result body has exact key order `input_sha256,sample_noise_sha256,problem_evidence,objective,storage,fill,workspace,counters,allocation,resources,diagnostics,operation_reachability,residuals,resource_decisions,invariants`. Negative-control results use the same v2 envelope but their control-result body omits and never fabricates production `problem_evidence`.
- The parent decodes a trusted production/profiler v2 PASS body into a separate private typed `H8DecodedPassEvidence` attached as `H8ChildAttemptRecord.pass_evidence`. Its only fields are `sample_noise_sha256`, typed `problem_evidence`, typed `condition_diagnostics`, the complete parent-owned/frozen/validated child `allocation` mapping, and the complete parent-owned/frozen `child_identities` mapping. `pass_evidence` is required for trusted production/profiler PASS and is null for controls and every non-PASS attempt. It is private/internal and omitted as a direct field from `_attempt_payload` and validation `child_attempts`; it does not duplicate fill, workspace, counters, resources, or reconstruction data retained publicly or on the attempt.
...
- Allocation certification has four required primary channels: PyTorch dispatch whitelist/stack/live-storage tracing, a separate PyTorch profiler run that preserves and reconciles all four raw timeline actions `PREEXISTING`/`CREATE`/`INCREMENT_VERSION`/`DESTROY`, backend counters, and clean-subprocess OS high-water memory. `tracemalloc_supplementary` is literal JSON `null`, meaning "not collected"; it is excluded from `all_observable`, every memory decision, and overall status and cannot close, rescue, or override a claim. The documented lossy raw-export rows and profiler aggregate/net memory deltas are supplementary only and cannot close an allocation claim.
...
- Reconstruct each of the three seed-level problem records only from problem evidence that is byte-identical across all five production repetitions and that seed's profiler run; disagreement is `INCONCLUSIVE`, and first-run selection is forbidden. `generative_sha256` and `recognition_sha256` are domain-separated canonical-JSON hashes under `h8-generative-evidence-v1` and `h8-recognition-evidence-v1`; arrays retain exact shape, little-endian float64 dtype, and raw C-order byte hash. `local_spd_diagnostics` uses `h8-local-spd-diagnostics-v1` and retains every named local NumPy-Cholesky diagonal pivot plus the global minimum. `transition_norms` uses `h8-transition-norms-v1`, literal `norm="operator_2"`, every ordered model/state/coupling/recognition single-pass ideal contraction-norm witness array and each maximum. Each witness is exactly `min(radius,raw_norm)` from the sole finite nonnegative raw matrix operator-2 norm already required by generator contraction; it is not the exact operator-2 norm of the rounded stored matrix, and no second per-child SVD is allowed. `observation_sha256` hashes canonical JSON `{"domain":"vfe4.h8.observations.v1","records":[[receiver_t,x_t],...]}` in receiver order.
- The parent owns the frozen ordered inventory of at most 30 `H8ChildAttemptRecord` entries: 15 production attempts in seed-major/repetition order, three profiler attempts in seed-major order, then 12 negative-control attempts in the frozen control order using seed `20260721`. Every launch actually attempted is retained even when it times out, exits abnormally, emits malformed output, or never yields an `H8ChildResult`/`H8ControlResult`. Every attempt retains the exact request, attempt status/reasons, optional typed result identity, an immutable parseable envelope from a timed-out launch or a parseable non-PASS/identity-rejected envelope when available, timeout/exit facts, actual parent elapsed nanoseconds, request/identity/stdout/stderr hashes, and optional trusted raw reachability/residual/resource decisions. The decoded `production_runs`, `profiler_runs`, and `controls` inventories remain separate and must cross-bind exactly to the result-bearing attempts; they cannot substitute for or synthesize a missing attempt. Witnessed failure may close H8 as FAIL before all 30 launches; PASS requires all 30. Parent timing is measured and retained at `attempt.parent_elapsed_ns`, is copied into the expanded published production/profiler record's separate top-level `parent_elapsed_ns`, and never overwrites the child-authored `result.resources.parent_elapsed_ns=0`.
- After config and prerequisites have established a valid child start, any witnessed attempt timeout, OOM/abnormal or nonzero child exit, forbidden allocation/operation, off-band fill, nonfinite contract value, solver inability, completed-run omission of a required operation, reported thread/environment identity mismatch, over-budget finite residual/resource endpoint, or executed negative-control miss dominates as `FAIL`, even if later evidence is unavailable. Only unavailable or unwitnessed evidence, missing observability/hashes, stale prerequisites, or a changed interpretation is `INCONCLUSIVE`; missing later evidence cannot mask a witnessed violation. `PASS` requires all 30 exact ordered attempts with request/result cross-binding, all 15 decoded production runs, all three decoded profiler runs, all 12 decoded controls, all 12 correctness cells, every control/channel, every numerical/resource budget, and every required operation invariant.
- `validation/h8.json` is the normative in-artifact structured result under `schema_version="h8-sparse-scale-v5"`. The 2.9.1/v4 contract is superseded pre-outcome history; v5 binds exact source revision and dirty-content digest, canonical v3 config hash, prerequisite artifact/hash set, manuscript/preregistration hashes, parent-owned child attempts, v3 child protocol and installed-runtime environment, raw run/correctness/allocation records, statuses/obligations, and explicit H8 nonclaims. It contains no hash of its own enclosing manifest. Only after atomic publication, write `.verification/h8-current-candidate-<FULL_HEAD>-result.json` with the artifact path plus candidate, manifest, config, validation, JUnit, reference-registry, and predecessor hashes; the H8 ledger must re-read and independently revalidate this external pointer.
...
- The tracked `verification/fixtures/h8_exact_test_nodes_v1.txt` owns every H8 test selection. Each new v5-contract test must first receive its exact pytest `::` node ID in that manifest; every RED, GREEN, or JUnit instruction consumes only the IDs assigned to its step. No instruction authorizes a whole test file or an unfiltered full suite. This documentation amendment authorizes no test and contributes no milestone evidence.
...
The superseded pre-outcome 2.9.1/v4 profiler pin is retained only as history. The active v5 profiler contract freezes the installed exact `torch.__version__="2.10.0.dev20251210+cu128"`, `torch/profiler/_memory_profiler.py` SHA-256 `22de3b0790907b90053af829ebf1bff0b6add2745ac0381ec7de78812edacb47`, `torch/profiler/profiler.py` SHA-256 `543430b2e9b24df777f86415865fee250b35e3444a80920bcca0e8889b917956`, and the 757-byte canonical descriptor SHA-256 `2ee166166bab997499cc66da85146a031f458fbe0190a75b1a1a3ddea80efc38`. Invoke `torch.profiler.profile(record_shapes=True,profile_memory=True,with_stack=True)`, retain that live profiler object, and in the same process read the private pinned `profile._memory_profile().timeline` rows `(timestamp_ns, Action, (TensorKey,version), numbytes)` plus `profile.profiler.kineto_results.experimental_event_tree()`. The descriptor freezes the symbols, tuple shapes, actions, joins, and flags; its full version and Python-source hashes do not identify compiled Kineto, for which the real schema preflight and scientific profiler child supply behavioral evidence. Do not claim that the documented `export_memory_timeline(...raw.json.gz)` row `(timestamp_ns,action,numbytes,category)` contains identity, operator, stack, or shape; preserve those exported rows only as a lossy secondary cross-check.

The following descriptor paragraph is superseded pre-outcome v4 history only and
is not an accepted or active profiler contract.

Enrichment is one deterministic in-memory join over the already captured timeline and event tree. Index `_EventType.Allocation/_ExtraFields_Allocation` nodes and `_EventType.TorchOp/_ExtraFields_TorchOp` tensor inputs by the full private `TensorKey` identity, then join timeline rows by that exact key and version. Persist the key exactly as `{tensor_id,storage_ptr,allocation_id,device}`; `CREATE`/`DESTROY` additionally require the unique allocation-event timestamp and signed byte-size witness, while `PREEXISTING` uses the first temporally ordered tensor-metadata occurrence and `INCREMENT_VERSION` uses the unique data-flow/TorchOp mutation at its timestamp. Derive logical shape/dtype from `_TensorMetadata`; derive operator and stack from the matched TorchOp and its event-tree ancestry. Hash source-row index, matched-node indices, key/version, and ancestry as the join witness. A missing, nonunique, or inconsistent identity/timestamp/bytes/version/operator/stack/shape join makes the profiler channel `INCONCLUSIVE`; no fallback to aggregate rows, nearest-name matching, or file reopen is permitted.
...
| `verification/h8_gate.py` | Prerequisite preflight, 12-cell correctness grid, frozen 30-request parent orchestration with retained attempt-prefix failure closure, decoded-result cross-binding, allocation/control certificates, status precedence, exact `validation/h8.json`, and complete H8 publication payload-map construction. |
...
| `tests/promotion/test_h8_gate.py` | Prerequisite, frozen-order attempt-prefix and PASS-complete ordering/cross-binding, decoded-run completeness, resource/allocation/status precedence, schema, stale-evidence, and nonclaim coverage. |
| `tests/integration/test_verify_vfe4.py` | H7 selected-operation path extended to reference current H7 plus separate executable Prediction-v3/v4-registry evidence, return the explicit H8 result variant, and publish H8 once. |
...
| `tests/unit/test_atomic_artifacts.py` | Exact H8 artifact/provenance paths, manifest, raw parent-attempt and decoded-child records, and prerequisite references. |
| `verification/fixtures/h8_exact_test_nodes_v1.txt` | Future tracked, ordered exact-node manifest created by the parent-orchestrator slice; no whole-file or unfiltered full-suite H8 selection. This amendment names but does not create it. |
...
class CurrentH8PrerequisiteRefs:
    ...
    h7: H8H7Reference
    # The latter two variants are parser history and never authorize H8.
    h6_prediction: H8H6PredictionV3Reference | H8H6PredictionReference | H8LegacyH6PredictionReference
    registry_sha256: str
...
def build_h8_publication_payloads(  # verification/h8_gate.py only
    ...
    *,
    ...
    h6_prediction_reference: H8H6PredictionV3Reference,
) -> Mapping[str, object]: ...
...
`H8DecodedPassEvidence` is private/internal and has only the five fields shown
above. Its `allocation` is the complete parent-owned, frozen, validated child
mapping, and `child_identities` is the complete parent-owned, frozen identity
mapping. `H8ChildAttemptRecord.pass_evidence` is required for every trusted
production/profiler PASS and is null for controls and every non-PASS attempt.
`_attempt_payload` and validation `child_attempts` omit `pass_evidence` as a
direct field. Fill, workspace, counters, resources, and reconstruction data
remain on the typed result or attempt and are not duplicated privately.

The implementation imports and reuses H7's exact frozen `H7PredecessorReference(artifact_path,git_head,dirty_digest,junit_sha256,junit_path,manifest_sha256,payload_hashes,ledger_path,ledger_sha256,reference_sha256)` and H7-owned `project_h1_h5_compatibility_config`, plus H6's exact frozen `CandidateArtifactReference`, `project_h1_prefix_prior_v2_config`, `project_h6_prefix_config`, keyword-only `run_projected_current_candidate`, and paired `reopen_h6_prefix_authorities`. H6-Prefix calls the one-argument projector and producer with `predecessor_refs={}`. `project_h7_compatibility_config` consumes the exact ordered H7 mapping without projection loss. Separately, H8-v5 retains complete keyed content/payload maps and all kind-specific identities. Its Prefix reference binds the six-file inventory and both `prefix_certificate_set_sha256` and `a0_direct_exact_prefix_certificate_sha256`; the paired reopener requires current-source direct-A0 PASS evidence with empty obligations, the complete passing check inventory, and membership of its bounded-A0 premise in the same certificate set. Every Prediction-v3 `content_hashes` key must be one exact safe POSIX manifest-relative name; absolute, drive-qualified, backslash, `.`/`..`, and nonmanifest names fail before reading. H8 reopens the exact H7 chain plus native Prediction-v3 authorities, validation/checkpoint selection, authenticated reservation and reservation-derived RESULT/TERMINAL/FINALIZED/pointer paths, raw inventory, metrics, result, and candidate JUnit. Config and readiness must both bind the registry's direct-A0 certificate SHA-256 before any downstream result read. It separately pins the installed verification validator to tracked SHA-256 `a8a799496762910c463ecc179a4d63dc40107fcbe81553add189de7ed1ce4c95`; a registry hash cannot select another validator. The exact closure claim ID is `h6-prediction-v3-exact-artifact-closure`. Its mechanical evidence locations are canonical compact JSON strings `{"path":"<absolute-forward-slash-path>","sha256":"<lowercase-64-hex>"}` for the candidate JUnit, `result.json`, result manifest, authorities manifest, validation manifest, reservation marker, TERMINAL manifest, FINALIZED manifest, and pointer manifest. H8 bounded-reads and rehashes each preimage and requires those exact strings in that specific nonempty experiment closure claim at the live artifact revision. Prediction-v1/v2 types and the five-file Prefix-v4 type remain readable only in the historical parser branches and may not reach this adapter. `verification/h8_gate.py` owns H8 reference validation and payload construction; `verification/run_gates.py` hands that mapping to the generic atomic publisher. No module under `vfe4/**` imports `verification/**`.
...
The sole authorizing registry is `h8-current-candidate-refs-v5`. Its direct
H1--H5, H1-prefix-prior, and H6-Prefix variants equal the H7 transitive
references field-for-field. The Prefix variant also binds the exact direct-A0
certificate alongside the bounded set. Its `H8H6PredictionV3Reference` retains the exact
producer head/dirty digest/artifact revision, JUnit path/hash, and installed
ledger-validator source hash, which need not equal the H8 candidate. Native
reopeners authenticate config/matching/readiness/plan authorities, validation
and checkpoint selection, the RESERVED record and its exact derived result,
TERMINAL, FINALIZED, and pointer paths, then raw inventory, metrics, result,
and pointer identities. The parsed ledger must pass the source-hash-bound
installed deterministic validator in schema 1.0 closure mode at the live
artifact revision, with a nonempty all-`EVIDENCE_VERIFIED` claim inventory,
no open obligation or invalidation, and one claim bound to the exact result
manifest/result/JUnit preimages. Missing or changed preimages reject registry
v5 rather than creating a self-consistent authorization. Registries v1, v2,
v3, and v4 decode only for diagnosis. Registries v1--v3 contribute
`h8_prerequisite_registry_v1_v2_v3_requires_h6_prediction_v3`; all four
contribute
`h8_prerequisite_registry_v1_v2_v3_v4_requires_direct_a0_prefix`; v1/v2 also
retain the legacy H6-Prefix shape and its bounded-Prefix obligation.
...
- [ ] **Step 2: Run only the new layout tests and confirm RED.**
  ...
  Run only the exact Task 1 RED node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  layout test file.
...
- [ ] **Step 4: Write RED conditional-config and lifecycle-consumer tests.** Preserve every accepted shorter verifier prefix and add only the exact H8 prefix. Freeze H6-Prefix's one-argument projector, keyword-only producer, and `predecessor_refs={}`. Require registry v5's exact six-file `H8H6PrefixReference`, including the direct-A0 certificate digest, and the separate `H8H6PredictionV3Reference`: exact authority/readiness/plan/matching, validation/checkpoint, reservation-derived outputs, result inventories, pinned validator, live revision, hash-bearing closure claim, and JUnit identities. Reject every v1/v2 Prediction variant and every v1--v4 Prefix/registry variant before effects, along with missing, stale, reordered, aggregated, escaped-name, alternate-validator, or path-only evidence records.

- [ ] **Step 5: Implement conditional H8 config resolution, pure H7 projection, current-reference binding, and canonical hashing.** Preserve the H7 projection interfaces and exact H6-Prefix producer. `bind_h8_current_refs` accepts only registry v5 with the same-candidate H7 chain, exact paired bounded/direct-A0 Prefix authority, and exact executable Prediction-v3. It preserves every keyed identity, requires the tracked pinned validator digest rather than trusting the registry's choice, and sends v1--v4 parser variants directly to named historical nonauthorization obligations. Artifact identities remain references, never copied PASS booleans or payloads.

- [ ] **Step 6: Author and freeze the preregistration before any H8 calculation.** Copy the global constraints, operational (not theorem-level) `K=d_z=d_m=20` choice and `interpretation_sha256`, exact source/H7-plan hashes, exact `torch==2.9.1` profiler-source/API hashes, literal problem RNG pseudocode plus ordered draw-schema descriptor/hash and serialization point, literal 12-cell problem/noise seed table, exact storage arithmetic, algorithms, budget constants/formulas, frozen 30-request plan (15 production in seed-major/repetition order, three profilers in seed-major order, then 12 controls in frozen order using the first seed), retained attempt-prefix failure semantics, four observability channels, the four-action profiler liveness/join contract, every exact assigned negative control, status precedence, exact discriminated reference/external-result schemas, full payload schema, and nonclaims. Include the block selected-inverse derivation and the reason sparse precision does not imply sparse covariance.

- [ ] **Step 7: Run focused GREEN tests.**

  Run only the exact Task 1 GREEN node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select either
  whole test file.
...
- [ ] **Step 2: Run the new backend test file and confirm RED.**
  ...
  Run only the exact Task 2 RED node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  backend test file.
...
- [ ] **Step 8: Run focused GREEN tests.**
  ...
  Run only the exact Task 2 GREEN node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  backend test file.
...
- [ ] **Step 9: Run focused GREEN tests.**
  ...
  Run only the exact Task 3 GREEN node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  objective test file.
...
- [ ] **Step 7: Run focused GREEN oracle tests.**
  ...
  Run only the exact Task 4 GREEN node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  oracle test file.
...
- [ ] **Step 8: Run focused GREEN tests.**
  ...
  Run only the exact Task 5 GREEN node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  allocation test file.

  Expected: PASS with all assigned controls detected and benign block operations accepted.

- [ ] **Step 9: Review and commit Task 5.** Reviewer searches for unguarded constructors/conversions, checks alias accounting and `finally` restoration, and confirms `tracemalloc_supplementary` is literal JSON `null` and excluded from observability, memory decisions, and status. Commit `feat(h8): certify sparse allocations`.
...
- [ ] **Step 7: Run focused GREEN child tests.**
  ...
  Run only the exact Task 6 GREEN node IDs assigned in
  `verification/fixtures/h8_exact_test_nodes_v1.txt`; do not select the whole
  allocation test file.

  Expected: PASS for the focused GREEN child tests. Any bounded development,
  unit, or fake-child execution in this step is non-authorizing and is not
  milestone evidence; no v5 scientific/milestone execution from it may be
...
- Create: `verification/fixtures/h8_exact_test_nodes_v1.txt`
- Modify: `README.md`
...
**Current staged state:** Task 7A froze the parent request plan and exact-node
manifest; Task 7B added the injected issued-prefix runner. No v5
scientific/milestone execution has been accepted as promotion evidence. Prior
bounded development, unit, and fake-child tests are non-authorizing and are
not milestone evidence. Remaining work must execute strictly in this order:
**7C1** lossless typed child evidence and exact problem endpoints; **7C2** an
H8-owned runtime-section builder plus full independent comparison; **7C3**
selected H8 runner/click-run integration followed, and only followed, by
removal of the three current PASS locks; then **7D** focused exact-node JUnit
verification and review. A later phase cannot weaken or bypass an earlier
phase.

**2026-07-27 installed-runtime amendment:** The remaining v4/config-v2
implementation-detail lines in this archived task sequence are superseded
pre-outcome history only. The active requirements are the v5/config-v3/
`vfe4.h8.parent-child-protocol.v3` contract at the top of this plan and in the
amended preregistration; `h8-child-v2` remains unchanged as the wire envelope.

- [ ] **Step 1: Write RED prerequisite tests.** Require the exact current H7 chain without copying predecessor payloads. Separately require `H8H6PredictionV3Reference` and native reopen of its authority bundle, validation/checkpoint selection, authenticated reservation, reservation-derived RESULT/TERMINAL/FINALIZED/pointer, raw inventory, metrics, result, candidate JUnit, and exact closure ledger. Recompute each referenced file or directory-manifest digest and require canonical hash-bearing evidence locations in the specific Prediction-v3 closure claim. Reject historical Prediction parsers, alternate self-consistent validators, path-only evidence, any artifact mutation, or any H4-timing premise before children launch.

- [ ] **Step 2: Preserve the completed parent-orchestration baseline.** The tracked `verification/fixtures/h8_exact_test_nodes_v1.txt` contains ordered exact pytest node IDs and rejects duplicate or whole-file entries. The parent traverses seed order `20260721..23`, repetitions `0..4`; creates a fresh process for every production run; runs one separate profiler child per seed; then runs each negative control in isolation in frozen order with seed `20260721`. It preallocates the exact 30-request sequence and emits one `H8ChildAttemptRecord` for every launch actually issued, including timeout, abnormal exit, malformed stdout, and missing typed result. A witnessed FAIL may stop later launches and leave the exact issued prefix; PASS requires all 30. Enforce parent timeout `60.0`; reject duplicate/reordered attempts, a non-prefix inventory, warm reuse, a result from another request/config/protocol hash, any decoded result not cross-bound to its result-bearing attempt, or aggregation that discards any `PREEXISTING`/`CREATE`/`INCREMENT_VERSION`/`DESTROY` source row, TensorKey+version/dtype/join witness, preexisting baseline field, or raw endpoint.

#### Task 7C1 — Lossless typed child evidence and exact problem endpoints

- [ ] **Step 3: Implement lossless child evidence and exact problem endpoints.** Bump `H8ValidationConfig` to `h8-validation-config-v2` with the six frozen protocol fields and require its changed canonical config SHA. Bump the envelope to `h8-child-v2` and bind that literal into the parent/child protocol digest. Every production/profiler result body has exact key order `input_sha256,sample_noise_sha256,problem_evidence,objective,storage,fill,workspace,counters,allocation,resources,diagnostics,operation_reachability,residuals,resource_decisions,invariants`. Negative-control result bodies remain under the same v2 envelope but omit and never fabricate production `problem_evidence`. Decode each trusted production/profiler v2 PASS body into the private `H8ChildAttemptRecord.pass_evidence`. The exact frozen/slotted `H8DecodedPassEvidence` has only `sample_noise_sha256: str`, `problem_evidence: H8ProductionProblemEvidence`, `condition_diagnostics: SparseConditionDiagnostics`, `allocation: Mapping[str, object]`, and `child_identities: Mapping[str, object]`. The allocation is the complete parent-owned, frozen, validated child mapping, and the identities are the complete parent-owned, frozen child identities. Require `pass_evidence` for trusted production/profiler PASS and set it to null for controls and every non-PASS attempt. Omit this private/internal field directly from `_attempt_payload` and validation `child_attempts`; do not duplicate fill, workspace, counters, resources, or reconstruction data already retained by the typed result or attempt. Keep typed `H8ChildResult` and `H8GateResult.production_runs`/`profiler_runs` unchanged, preserve the existing expanded published validation records, and never append private evidence directly to them. Derive the `problems`, factor, and allocation runtime sections from private evidence combined with `H8ChildResult` and attempt endpoints. Never reduce private allocation evidence to `H8AllocationRecord`, select the first run, retain only a maximum, or rewrite `resources.parent_elapsed_ns=0`. Persist profiler keys as `{tensor_id,storage_ptr,allocation_id,device}`. Freeze the canonical, key-sorted `h8-generative-evidence-v1`, `h8-recognition-evidence-v1`, `h8-local-spd-diagnostics-v1`, `h8-transition-norms-v1`, and observation hash preimages exactly as specified in v4. Capture transition norms from the already required contraction calculation without a second per-child SVD pass. Reconstruct each seed's problem record only after byte-identical agreement across all five production repetitions and that seed's profiler result; disagreement is `INCONCLUSIVE`.

  The exact key-sorted canonical JSON objects hashed by
  `generative_sha256` and `recognition_sha256` are:
  ...
  Every elided transition/emission array and every explicit array leaf is
  exactly `{shape,dtype:"<f8",raw_sha256}`. `shape` and `source_support` are
  JSON arrays of integers; all identity, dimension, seed, vocabulary,
  receiver, parent, and observation values are JSON integers.

- [ ] **Step 4: Write and implement exact status precedence.** First validate config/interpretation/prerequisite/preregistration identities sufficiently to establish whether a child/control start is in scope. Thereafter, a witnessed attempt timeout, OOM/abnormal or nonzero exit, forbidden allocation/operation, off-band fill, nonfinite contract value, solver inability, completed-run omission of a required operation, reported thread/environment identity mismatch, finite residual/resource/pivot breach, invalid profiler dedup/version/liveness transition, or executed-control detector miss immediately dominates as `FAIL`, even if a later event, stack, environment field, or result fragment is unavailable. A failed/malformed launch still retains its parent-owned attempt record and does not require a fabricated typed child result. In the absence of a witnessed violation, map unavailable/unwitnessed evidence, including a missing typed result, any missing/nonunique profiler event-tree join or source/TensorKey/version/dtype witness, incomplete observability/control assignment, missing hash, stale prerequisite, or ambiguous interpretation to `INCONCLUSIVE`. Evaluate completeness/decisiveness, the exact ordered 30-attempt inventory, request/result cross-binding, all four profiler actions, storage/RHS/operation reachability, and the decoded run/control conjunction only after applying that dominance rule. Permit `PASS` only when all 30 attempts are present, ordered, cross-bound, and PASS, every decoded inventory is complete, `every_profiler_action_joined_and_liveness_reconciled` is true, and every other named invariant is known true.

#### Task 7C2 — Owned runtime-section builder and independent comparison

- [ ] **Step 5: Build, independently compare, and freeze `validation/h8.json`.** The H8 gate owns runtime-section construction from the exact typed child/problem evidence; no caller-supplied or preassembled runtime mapping can authorize status. Independently reconstruct and compare every field, run key, hash, schema literal, inventory order, resource sentinel, and cross-binding before publication. Set `schema_version="h8-sparse-scale-v4"` and `gate="H8"`. Reject unknown/missing keys at every level. The exact top-level key order is `schema_version`, `gate`, `status`, `obligations`, `bounded_claim`, `nonclaims`, `revision`, `config`, `prerequisites`, `interpretation`, `protocol`, `environment`, `problems`, `storage`, `factor`, `correctness`, `allocation`, `controls`, `child_attempts`, `production_runs`, `profiler_runs`, `budgets`, `invariants`, and `artifacts`. Exact nested key sets are:

  - `revision={git_head,dirty_digest,dependency_closure_sha256,manuscript_sha256,preregistration_sha256,h7_plan_sha256}`. `manuscript_sha256` is exactly `d733880d3613d32a97b7a12c93ff6c037d0abdfd9ce4810e411769997dbad03c`, the raw-byte digest of `Manuscripts/VFE4_gauge_causal_elbo_whitepaper.tex`; no MAgent manuscript enters this field;
  - `config={config_sha256,objective_schema_sha256,protocol_sha256,canonical_json_sha256,selected_operation,ordered_gates,current_refs_registry_sha256,candidate_junit_sha256}`. Both config hashes bind exact `h8-validation-config-v2`, including the six frozen factor/selected-inverse/condition/allocation/profiler-event/child schema fields; the canonical config SHA therefore differs from the v1 identity;
  - `prerequisites={h7_compatibility_refs,h1_h5,h1_prefix_prior,h6_prefix,h7,h6_prediction,compatibility_checks,obligations,all_current_and_pass}`. `h7_compatibility_refs` is the canonical serialization of the exact ordered `Mapping[str,H7PredecessorReference]`, including each complete keyed `payload_hashes` mapping and JUnit SHA. The other five values are, verbatim and with no extra or omitted field, canonical serializations of `H8H1H5Reference`, `H8H1PrefixPriorReference`, `H8H6PrefixReference`, `H8H7Reference`, and `H8H6PredictionV3Reference`, respectively. The H6-Prediction variant preserves exact authority/readiness/plan/matching, validation/checkpoint-selection, reservation/output namespace, raw-inventory/metrics/result, validator/artifact-revision, and JUnit bindings. Keyed `content_hashes`/`payload_hashes` and the kind-specific certificate-set/certificate, result-pointer/fixture-set, and experiment fields remain explicit. No singular `content_sha256` or `result_or_certificate_sha256` reduction is allowed, and H8 stores references only rather than copying validation, certificate, or ledger bytes;
  ...
  - `protocol={generator_schema,generator_draw_schema_sha256,sample_schema,factor_schema,selected_inverse_schema,condition_estimator_schema,allocation_schema,torch_version,profiler_source_hashes,profiler_api_contract_sha256,profiler_raw_event_schema,child_schema,production_seed_order,production_sample_seed_map,repetition_order,correctness_seed_table,required_operations,negative_control_order}`. Require `factor_schema="h8-block-tridiagonal-cholesky-v1"`, `selected_inverse_schema="h8-block-takahashi-selected-inverse-v1"`, `condition_estimator_schema="HagerHigham1NormEstimate-v1"`, `allocation_schema="h8-allocation-observability-v1"`, `profiler_raw_event_schema="h8-torch-profiler-raw-event-v1"`, and `child_schema="h8-child-v2"`. The canonical parent/child protocol digest preimage includes all five evidence-schema literals, the child-envelope literal, complete required-operation and negative-control inventories, and every frozen numerical and boundary constant;
  ...
  - `problems` is an ordered three-record array with each record exactly `{problem_seed,sample_noise_seed,input_sha256,sample_noise_sha256,generative_sha256,recognition_sha256,local_spd_diagnostics,transition_norms,observation_sha256}`. `generative_sha256` is the domain-separated SHA-256 of canonical JSON for `h8-generative-evidence-v1`, covering layout/seed/vocabulary metadata, `alpha`, initial mean/covariance, every model/state transition, and every emission. Every array is represented by exact shape, little-endian float64 dtype, and raw C-order byte SHA-256; transition identities, source supports, and observations stay explicit. `recognition_sha256` analogously hashes `h8-recognition-evidence-v1` over the recognition initial mean/covariance and every ordered recognition transition. `local_spd_diagnostics` has `schema_version="h8-local-spd-diagnostics-v1"` and retains the minimum NumPy-Cholesky diagonal pivot for the generative initial covariance, every ordered model covariance, every ordered state covariance, the recognition initial covariance, every ordered recognition covariance, and the global minimum; these local problem-covariance diagnostics are distinct from production block-factor pivots. `transition_norms` has `schema_version="h8-transition-norms-v1"`, `norm="operator_2"`, ordered single-pass ideal contraction-norm witness arrays for model, state, state-model coupling, and recognition matrices, and each inventory maximum. Each witness is exactly `min(radius,raw_norm)`, with finite nonnegative `raw_norm` from the sole raw matrix operator-2 SVD already required by contraction. It is not the exact operator-2 norm of the rounded stored matrix; a second per-child SVD is forbidden. `observation_sha256` is the domain-separated SHA-256 of canonical JSON `{"domain":"vfe4.h8.observations.v1","records":[[receiver_t,x_t],...]}` in receiver order. Reconstruct each seed record only after byte-identical problem evidence across all five production repetitions and that seed's profiler run; disagreement is `INCONCLUSIVE`, and first-run selection is forbidden;
  ...
  - `factor={schema_version,algorithm,pattern,runs}`, with `schema_version="h8-factor-evidence-v1"`, `algorithm="block_tridiagonal_cholesky_local_recursion"`, and `pattern="symmetric_block_tridiagonal_diag_lower_only"`. `runs` is exactly 18 run-keyed records in authoritative `production_runs` then `profiler_runs` order, each `{mode,seed,repetition,input_sha256,fill,workspace,condition_diagnostics,counters,reconstruction_invariants}` and cross-bound to the corresponding result-bearing attempt. `condition_diagnostics` retains the complete typed `SparseConditionDiagnostics`, including the diagnostic-only condition estimate and every per-block/global pivot endpoint. No first-run selection, maximum-only reduction, or silent cross-run collapse is allowed;
  ...
  - `allocation={schema_version,whitelist,runs,tracemalloc_supplementary,all_observable,no_forbidden_attempts}`, with `schema_version="h8-allocation-evidence-v1"`. `runs` is exactly the same 18-record authoritative order, each `{mode,seed,repetition,input_sha256,allocation,resources}` and cross-bound to the corresponding result-bearing attempt. `allocation` is the complete validated child allocation mapping, not the reduced `H8AllocationRecord`; it retains dispatch events/scopes/storage, NumPy inventory/guard events, profiler API/raw events/lossy supplementary rows, preexisting baseline and liveness reconstruction, and raw dispatch/backend cross-checks. Each profiler record retains `{source_row_index,timestamp_ns,action,tensor_key,version,nbytes,dtype,device,operator,stack,logical_shape,classification,matched_event_node_indices,join_witness_sha256,live_bytes_after}`, with exact `tensor_key={tensor_id,storage_ptr,allocation_id,device}`. `resources` is the exact child-authored `H8ResourceRecord`, including `parent_elapsed_ns=0`; actual parent elapsed time is retained in the attempt and in the expanded published run record's separate top-level `parent_elapsed_ns`. `tracemalloc_supplementary` is literal JSON `null`, meaning "not collected"; it is excluded from `all_observable`, every memory decision, and overall status and cannot close, rescue, or override a claim;
  ...
  - `child_attempts` is the parent-owned frozen-order prefix, up to the exact 30-record inventory: 15 production requests in `(seed,repetition)` seed-major order, three profiler requests in seed-major order, then 12 negative-control requests in the frozen control order with `seed=20260721` and `repetition=null`. A witnessed FAIL may close the array early; PASS requires all 30. Each `_attempt_payload`/validation attempt is exactly `{request,status,reasons,result_kind,result_identity,nonpass_envelope,timed_out,exit_code,parent_elapsed_ns,request_sha256,identities_sha256,stdout_sha256,stderr_sha256,operation_reachability,residuals,resource_decisions}`; the private/internal `pass_evidence` field is deliberately omitted from this direct publication surface. `result_kind` is `child`, `control`, or null; the corresponding optional identity is exactly `{mode,seed,repetition,input_sha256}`, `{control_id,event_sha256}`, or null. A parseable envelope from a timed-out launch, a parseable child-authored FAIL/INCONCLUSIVE envelope, or a parseable envelope whose identity is rejected, is retained immutably in `nonpass_envelope`; malformed or unavailable stdout uses null and remains bound by `stdout_sha256`. `request_sha256` binds the canonical stdin request, `identities_sha256` binds the exact canonical `VFE4_H8_CHILD_IDENTITIES_JSON` bytes, and the stdout/stderr hashes bind the raw captured streams even when parsing fails. Reachability, residual, and resource-decision maps are optional trusted raw evidence and use null when unavailable, inapplicable, or identity-rejected; missing required evidence contributes the appropriate attempt/gate reason rather than a fabricated map;
  - Typed `H8GateResult.production_runs` is the exact 15-record seed-major `H8ChildResult` tuple and typed `H8GateResult.profiler_runs` the exact three-record seed-major `H8ChildResult` tuple; these typed public contracts remain unchanged. The published validation `production_runs` and `profiler_runs` records preserve the existing expanded schema: every `H8ChildResult` field plus top-level `parent_elapsed_ns`, `child_elapsed_ns`, `exit_code`, `stdout_sha256`, `stderr_sha256`, `operation_reachability`, `residuals`, and `resource_decisions`. Private `H8DecodedPassEvidence` is not appended directly to those run records. The `problems`, factor `runs`, and allocation `runs` sections are derived from private evidence combined with `H8ChildResult` and attempt endpoints and cross-bound to the authoritative typed arrays and expanded published records; they do not replace either inventory. `controls` remains the exact 12-record decoded `H8ControlResult` array under the same v2 envelope without fabricated production `problem_evidence`. Each nested `resources` object remains the child's exact public output, including `resources.parent_elapsed_ns=0`; actual parent spawn-through-parse timing is retained on the attempt and in the expanded published record's separate top-level `parent_elapsed_ns`;
  ...
  - `invariants={prerequisites_current_and_pass,interpretation_hash_current,correctness_cells_complete,correctness_pass,controls_complete,child_attempts_complete,child_attempts_exact_order,child_attempts_cross_bound,observability_complete,every_profiler_action_joined_and_liveness_reconciled,production_runs_complete,profiler_runs_complete,required_operations_reached,storage_pass,forbidden_attempts_zero,offband_fill_zero,pivot_margin_pass,rhs_width_pass,sample_width_pass,time_pass,process_memory_pass,torch_memory_pass,finite_pass,residuals_pass,witnessed_failure_dominance_applied,all_pass}`;
...
#### Task 7D — Focused exact-node JUnit verification and review

- [ ] **Step 9: Run one focused tracked exact-node JUnit selection.**

  First add every new v5-contract test as an exact node ID in the tracked
  `verification/fixtures/h8_exact_test_nodes_v1.txt`. Run only the exact Task
  7C1/7C2/7C3 IDs assigned to Task 7D, in one focused pytest process with one
  JUnit XML. Every selected entry must contain a pytest `::` node delimiter;
  do not select an entire test file, a whole-file alias, or an unfiltered full
  suite. Derive totals only from the XML.

  Expected: PASS using small injected child attempts/results; this is not
  production H8 evidence.

- [ ] **Step 10: Review and commit Task 7.** Reviewer checks lossless typed child evidence; exact problem evidence and cross-run agreement; v4 factor/allocation views; literal schema/digest bindings; witnessed-failure dominance; full independent runtime-section comparison; removal of the three PASS locks only through the selected H8/click-run path; exact payload and external-pointer schemas; absence of an enclosing-manifest hash; H7/H6-Prediction freshness and ledger preservation; `H8GateResult` placement/closed union; verification-owned payload construction; gate-agnostic generic artifact publication; exact-node-only JUnit evidence; atomicity; one CONFIG; and nonclaims. Commit `feat(h8): add sparse scale promotion gate`.
...
- [ ] **Step 1: Freeze the final H8 source before producing evidence.** Record a full 40-character `HEAD`; require every Task 1--7 file tracked, including `verification/fixtures/h8_exact_test_nodes_v1.txt`; require every manifest entry to be an exact pytest node ID rather than a whole-file selector; require empty tracked and index diffs and no nonignored untracked path outside `.verification/`; verify the pinned final-H7 plan SHA; recompute dirty-content, exact-node-manifest, H8 dependency, manuscript, preregistration, interpretation, config, objective, allocation, child-attempt/child, and seed-table hashes; validate the unchanged separate H6-Prediction artifact/ledger and prove its scientific dependency closure is unaffected; require no `.verification/active.json`; hash `.verification/ledger.json` plus every historical ledger/artifact reference into `C:\tmp\vfe4-h8-current-candidate-preflight.json`. Require the JUnit destination and every new artifact/registry/result-pointer/ledger path to be absent rather than overwritten. `verification/h8_preflight.py` performs this metadata inspection without launching a test, correctness cell, runtime child, profiler, control, or scientific gate. Historical H1--H7 evidence is bootstrap context only and cannot count as current closure.

- [ ] **Step 2: Run the single exact-revision JUnit milestone once.** Resolve the manifest's frozen ordered milestone node IDs and run exactly those nodes in one pytest process with one XML output. Reject any whole-file entry and do not precede or follow it with another H8 pytest selection:

  ```text
  exact nodes from verification/fixtures/h8_exact_test_nodes_v1.txt
  plus: -q --junitxml=C:\tmp\vfe4-h8-milestone.xml
  ```

  Expected: exit 0. Parse totals, failures, errors, skips, and duration from XML only. If it fails, preserve XML/logs, fix on a new committed revision, run only the affected exact manifest nodes for the fix, and then run one replacement manifest milestone XML for that new revision; never combine totals across revisions.
...
- [ ] **Step 8: Atomically bind the exact H8 current-reference registry.** Only after the H7 ledger validates, construct from the already validated Step 6 records, without reopening any source or reconstructing a reference from artifact bytes, the exact tagged `H8H1H5Reference`, `H8H1PrefixPriorReference`, `H8H6PrefixReference`, `H8H7Reference`, and `H8H6PredictionV3Reference`. Atomically write `.verification/h8-current-candidate-<FULL_HEAD>-refs.json` with `schema_version="h8-current-candidate-refs-v5"`, exact candidate HEAD/digest/JUnit SHA, the exact H7 compatibility mapping, and those ordered variants. Preserve every keyed content/payload map, all bounded Prefix aggregate/family identities, the direct-A0 certificate identity, and every Prediction-v3 authority, checkpoint, reservation-derived output, validator, live artifact-revision, result, and JUnit identity. Recompute the registry SHA, call `bind_h8_current_refs`, and require that all current-candidate identities match Step 2 while the paired Prefix authorities and H6-Prediction pass their exact reopen contracts. Reopen the six Prefix payloads, native Prediction authorities, validation bundle, reservation, derived RESULT/TERMINAL/FINALIZED/pointer paths, result payloads, candidate JUnit, and parsed closure ledger; load the installed ledger validator only after its bytes match `ledger_validator_sha256`, capture the live producer revision, and require one all-verified identity-bound closure. Any missing, copied, reordered, aggregated, escaped content name, changed byte, or historical registry v1/v2/v3/v4 blocks H8 and remains `INCONCLUSIVE`.

- [ ] **Step 9: Run H8 exactly once and write its external result pointer.** Run `C:/anaconda/python.exe verify_vfe4.py` with the committed H8 `CONFIG`; `main` resolves only the deterministic Step 8 registry. Expected: prerequisite statuses are PASS, `H8: pass` prints only if the full conjunction passes, and one artifact path prints. The parent freezes 30 request slots: 15 cold production children in seed-major/repetition order, three separate profiler children in seed-major order, then the 12 isolated controls in their frozen order using seed `20260721`. Preserve one parent-owned attempt record for every launch actually issued, including failed/malformed/timeout launches with no typed result, and do not rerun a seed/control. A witnessed FAIL may stop later launches; only the PASS path issues all 30. Require every decoded production, profiler, and control result to cross-bind exactly to its result-bearing attempt; PASS additionally requires the complete 15/3/12 inventories. Independently require byte-identical problem evidence across each seed's five production repetitions and profiler run, every lossless v4 factor/allocation run view, literal-null `tracemalloc_supplementary`, and every observed `PREEXISTING`/`CREATE`/`INCREMENT_VERSION`/`DESTROY` row with source index, exact `{tensor_id,storage_ptr,allocation_id,device}` plus version, dtype, join witness, and valid baseline/dedup/version/liveness reconciliation; a disagreement or unavailable/nonunique join is `INCONCLUSIVE`. Validate the generic atomic artifact, exact payload set, `H8GateResult`, `validation/h8.json` v5, config/validation hashes, the five embedded lossless reference variants, and manifest. Then atomically write `.verification/h8-current-candidate-<FULL_HEAD>-result.json` with those same variants verbatim plus candidate/JUnit, artifact/manifest/config/validation, and current-registry identities. It is not part of `manifest.sha256`.

- [ ] **Step 10: Have fresh H8 reviewers consume existing evidence only.** One reviewer checks block algebra/factor/selected inverse/pivots/objective/budgets; one checks the retained frozen-order attempt prefix (and the exact 30-attempt inventory for PASS), request/result/hash cross-binding, parent timing separation, all four profiler actions, TensorKey+version/source-row/dtype joins, preexisting baseline, dedup/version/liveness reconstruction, production whitelist, Windows HWM, controls, and resource endpoints; one checks the lossless registry -> H7 mapping -> H8 variants -> validation artifact -> external pointer round trip, H6-Prediction separation, and status dominance. They cite source, earlier exact-node outputs, the sole JUnit, current predecessor ledgers, and H8 artifact/pointer. They do not rerun tests or gates. Any Critical/Important fix creates a new candidate and complete replacement lifecycle.

- [ ] **Step 11: Revalidate the external pointer, then close the H8 ledger.** Re-read `.verification/h8-current-candidate-<FULL_HEAD>-result.json`; independently validate candidate HEAD/digest/JUnit, artifact path, manifest/config/validation, current-registry, every complete discriminated predecessor variant, byte-identical keyed maps/kind fields across the full round trip, and no enclosing-manifest hash in `validation/h8.json`. Derive `.verification/h8-<FULL_HEAD>-<H8_CONFIG_SHA>-ledger.json` from the validated artifact, require no active marker/path collision, and start one fresh closure ledger. Record one claim per check: complete prerequisite lifecycle/H6-Prediction separation; operational interpretation/hash; exact seed problem-evidence preimages and cross-run/profiler agreement; literal 12-cell three-way correctness; retained frozen-order attempt prefix and attempt/request/result/hash cross-binding, with exact 30-attempt completeness required only for PASS; preservation of child-authored `resources.parent_elapsed_ns=0` beside actual attempt timing; complete v4 factor/allocation run views; import boundary; operation vocabulary; input/factor/selected storage and pivot margins; all four profiler actions with source-row/tensor-key/version/dtype/join witnesses, preexisting counts/bytes/baseline, and dedup/version/liveness reconciliation; literal-null supplementary tracemalloc; every whitelist channel/control; zero fill; RHS/sample widths; every retained production elapsed/conservative-HWM/torch-peak endpoint, with all 15 required for PASS; hardware/affinity/thread/BLAS hashes; objective completeness; pointer/payload/manifest/JUnit identity; status precedence; and bounded nonclaims. Use current mechanical/reproduced evidence, required independent/skeptic/adjudicator views, and validate the exact ledger while source remains unchanged. Any join unavailable is `INCONCLUSIVE`; missing eligible evidence is never LLM consensus closure.

- [ ] **Step 12: Recheck immutability and report the complete evidence revision.** Recompute HEAD/digest, tracked/index diffs, all frozen hashes, exact-node manifest, both current registries, both external result pointers, all same-candidate artifacts/manifests/payload-or-certificate hashes, the separate H6-Prediction closure, five new ledger hashes, JUnit hash/XML totals, and every historical ledger hash. Expected: tracked source is unchanged; exactly the final-H7 ordered chain plus H8 evidence is new; every artifact shares the same candidate/JUnit except explicitly separate unchanged H6-Prediction; all five ledgers validate; and every older ledger is unchanged. Report exact revision/XML totals, predecessor paths/hashes, H6-Prefix certificate-set SHA, H7/H8 artifact and pointer paths, every retained attempt record (exactly 30 for PASS), every retained production raw endpoint/maxima (exactly 15 for PASS), pivot margins, correctness residuals only beside their own allowances, raw-event/control inventory, final status, and validated H8 ledger path. State explicitly that artifacts reference rather than copy predecessors and that parent timing did not rewrite child resources. Do not edit tracked documentation or rerun tests/gates after closure.
...
and child_attempts_complete == 30
and child_attempts_are_in_frozen_order
and every_attempt_request_hash_and_identity_hash_is_current
and every_result_bearing_attempt_is_cross_bound_to_its_decoded_result
and every_child_attempt_status_is_pass
...
Status evaluation is two-stage. Before a valid config/interpretation/prerequisite start is established, missing, stale, or failed prior-gate evidence blocks execution and yields H8 `INCONCLUSIVE`, preserving any prior gate's own `FAIL`. After a valid child/control start, the parent retains the corresponding attempt even if no typed result can be decoded. Any attempt with a witnessed timeout, OOM/abnormal or nonzero exit, forbidden allocation/operation, off-band fill, nonfinite contract value, solver inability, completed-run omission of a required operation, reported thread/environment identity mismatch, finite residual/resource/pivot breach, or executed-control missed detection is H8 `FAIL` and dominates missing later events, fields, or results. Only unavailable or unwitnessed evidence with no witnessed violation is `INCONCLUSIVE`; unknowns are never coerced to `False`, but they also never mask an observed failure. A decoded production/profiler/control inventory cannot fill a missing attempt, and an attempt identity cannot stand in for a missing decoded result required by PASS. The peak-to-peak HWM diagnostic, Hager--Higham estimate, literal-null `tracemalloc_supplementary`, and aggregate/net profiler deltas cannot override the conjunction.
...
- [ ] **Evidence lifecycle audit:** One exact-revision JUnit XML over the tracked exact-node manifest; then H1--H5, active H1-prefix-prior, independent H6-Prefix with `predecessor_refs={}`, H7 registry/artifact/external pointer/ledger, H8 registry/artifact/external pointer/ledger in exact order; one H8 execution; five new sequentially validated ledgers; separate unchanged H6-Prediction with H1/H2/H3/H5 and no H4 timing premise; the complete retained parent-attempt prefix and decoded-result cross-bindings preserved, with all 30 required for PASS; all four profiler-action records/baselines preserved; historical ledger hashes unchanged; and tracked docs frozen before candidate selection. Require the full registry -> exact H7 mapping -> discriminated H8 variants -> immutable-path revalidation -> validation artifact -> external pointer round trip to preserve keyed maps and kind fields byte-for-byte. No singular lossy hash, reference reconstructed from reopened bytes, copied predecessor payload/certificate/ledger, enclosing-manifest hash in `validation/h8.json`, whole-file/unfiltered suite selection, repeated manifest run, or post-evidence tracked edit.
...
- [ ] **Unfilled-marker scan:** No unset values, deferred thresholds, unnamed controls, unfrozen schema literals, or "similar to" steps remain.

## Execution Handoff

Plan implementation should proceed task-by-task with a fresh reviewer after each bounded commit. Final execution must use the pinned H7 plan and reproduce its current-candidate H1--H5, active H1-prefix-prior, independently produced H6-Prefix, and H7 artifact/pointer/ledger chain from the sole H8-candidate JUnit before H8 runs. H6-Prefix uses `project_h6_prefix_config(CONFIG)` and `predecessor_refs={}`. H6-Prediction remains separate executable v3 evidence, authorized only through the exact v5 paired-Prefix native-reopen and closure-ledger contract, and never H4 timing. Every downstream artifact binds predecessor identities without copying or rerunning predecessor payloads.
